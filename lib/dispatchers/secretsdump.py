...
class Dispatcher:
    ...
    def dispatch_st(self, module_name, module_data, module_args_raw, module_args_parsed, module_references):
        sam_filepath      = module_args_parsed.sam_file[0]
        security_filepath = module_args_parsed.security_file[0]
        system_filepath = module_args_parsed.system_file[0]

        if os.path.exists(sam_filepath):
            raise CoreException(f"invalid arguments '{sam_filepath}': file exists on filesystem")
        if os.path.exists(security_filepath):
            raise CoreException(f"invalid arguments '{security_filepath}': file exists on filesystem")
        if os.path.exists(system_filepath):
            raise CoreException(f"invalid arguments '{system_filepath}': file exists on filesystem")
        ...
        sam_data_hex, security_data_hex, system_data_hex = message.split(CUSTOM_SEPARATOR)
        sam_data      = hex2bin(sam_data_hex)
        security_data = hex2bin(security_data_hex)
        system_data   = hex2bin(system_data_hex)

        message = ""
        write_file(sam_filepath, sam_data, True)
        message += f"[+] Written SAM with {len(sam_data)} bytes to '{sam_filepath}'\n"
        write_file(security_filepath, security_data, True)
        message += f"[+] Written SECURITY with {len(security_data)} bytes to '{security_filepath}'\n"
        write_file(system_filepath, system_data, True)
        message += f"[+] Written SYSTEM with {len(system_data)} bytes to '{system_filepath}'\n"
