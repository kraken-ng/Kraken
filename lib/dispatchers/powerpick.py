...
class Dispatcher:
    ...
    def dispatch_c2(self, action, module_name, module_data, module_args_raw, module_args_parsed, module_references):
        ...
        if action == ACTION_LOAD:
            ...
            if (module_args_parsed.f == None) and (module_args_parsed.arguments == []):
                raise CoreException(f"invalid arguments. You must select a script or a command to be executed")

            if module_args_parsed.f == None:
                custom_args += "-"
            else:
                pwsh_script_filepath = module_args_parsed.f[0]
                if not os.path.exists(pwsh_script_filepath):
                    raise CoreException(f"invalid arguments '{pwsh_script_filepath}': file not exists")

                pwsh_script_raw  = read_file(pwsh_script_filepath, True)
                custom_args += bin2hex(pwsh_script_raw)

            custom_args += " "
            if len(module_args_parsed.arguments) > 0:
                custom_args += " ".join(module_args_parsed.arguments)
